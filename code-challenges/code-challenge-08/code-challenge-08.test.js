...
const filterStringsWithVowels = (array) => {
  return array.filter(i => i.match(/[aeiou]/ig));
};
...
const notInFirstArray = (forbiddenValues, array) => {
  let forbidden = array.filter( string => !(forbiddenValues.includes(string)))
  return forbidden;
};
...
const getBaseStatGreaterThan = (array, minBaseStat) => {
  let baseStat = array.filter( data => (data.baseStat > minBaseStat));
  return baseStat;
};
...
const getStatName = (array, minBaseStat) => {
  let outputArray = [];
  let baseStat = array.filter( data => (data.baseStat > minBaseStat));
  baseStat.forEach(element => {
    outputArray.push(element.stat.name)
  });
  return outputArray;
};
...
const getCharactersWithoutChildren = (array) => {
  let child = array.filter( data => !(data.children));
  return child;
};
