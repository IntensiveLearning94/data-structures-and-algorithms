...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; ++i) {
    theNewArray.push(Math.pow(2, arr[i]));
  }
  return theNewArray;
};
...
const forEachTwoToThe = (arr) => {
  let theNewArray = [];
  arr.forEach(numbers => {
    theNewArray.push(Math.pow(2, numbers));
  });
  return theNewArray;
};
...
const charCode = (arr) => {
  return arr.map(theNewArray => theNewArray.charCodeAt());
};
...
const extractAbilities = (arr) => {
  let newArray = [];
  arr.map((parameters) => {
    newArray.push(parameters.ability.name);
  });
  return newArray;
};
...
const extractStats = (arr) => {
  let newArray = []
  arr.map((allStats) => {
    newArray.push( {name:allStats.stat.name, total:allStats.effort + allStats.baseStat});
  });
  return newArray;
};
